...
6. A group image's tag list is the UNION of what the characters are doing: each pose, action, held-object and clothing-state tag (arms up, sitting, holding X, unworn swimsuit, bikini top only...) belongs to SOME character - usually exactly ONE, because the tagger tags what appears, not what everyone does. Copying one action to every character renders them as synchronized clones, which is almost always wrong. DISTRIBUTE instead: give each such tag to one character and spread the distinct actions/props/states so the characters differ - e.g. 3girls + arms up + innertube + holding swimsuit means ONE girl has her arms up, ANOTHER floats in the innertube, the THIRD holds the unworn swimsuit; not three girls with arms up. When you cannot tell whose action it is, a one-character guess still beats duplicating it (same principle as rule 5). Two exceptions: genuinely MUTUAL activities (dancing, hug, holding hands, kiss, eye contact, high-five...) involve all participants - keep those as interaction in base_prompt, never split or single them out; and expressions (smile, open mouth, blush) MAY repeat across characters when the scene reads that way. In NATURAL mode, scene_description says who does what ("one girl raises her arms; another lounges in a swim ring") - never claim everyone does the same thing unless a tag says so.
7. Do NOT add quality tags (masterpiece, best quality...). Tags stay lowercase, comma-separated, space form.
