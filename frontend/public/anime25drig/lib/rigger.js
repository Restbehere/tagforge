...
(function (root, factory) {
  ...
  function normName(n) {
    ...
    n = n.replace(/-(l|r)$/, '_$1');              // see-through --tblr_split suffixes
  ...
  function buildRig(psd, opts) {
    ...
    if (!kids.length) throw new Error('No layers found (groups are unsupported - flatten the layer structure)');
    ...
    if (faceE) {
      ...
      warnings.push('No "face" layer - assuming the canvas center is the face');
    ...
    for (var ei = 0; ei < entries.length; ei++) {
      ...
      var slot = SLOTS[bn];
      // Already side-split input (see-through --tblr_split): "eyewhite_l",
      // "handwear_r"... — map to the base slot instead of "unknown".
      var preSide = null, preBase = null;
      if (!slot) {
        var sm = /^(.*)_(l|r)$/.exec(bn);
        if (sm && SLOTS[sm[1]]) { slot = SLOTS[sm[1]]; preBase = sm[1]; preSide = sm[2]; }
      }
      if (!slot) {
        var c0 = centroidOf(e.alpha, W, H);
        slot = { depth: 1.0, group: (c0 && c0.cy < FACE.y1) ? 'head' : 'body' };
        warnings.push('Unknown layer name "' + e.name + '" - treating it as ' + slot.group);
      }
      if (slot.split && preSide) {
        // The layer is one side already — no split needed.
        var recS = makePart(e.name, e.layer, e.alpha, null, W, H, slot, z, preSide.toUpperCase(), null);
        if (recS) {
          parts.push(recS); z++;
          sided[preBase + '|' + preSide] = e.alpha;
        }
      } else if (slot.split) {
        ...
        if (!got) warnings.push('"' + e.name + '": left/right split failed (empty layer?)');
    ...
    if (mSrc) {
      ...
      warnings.push('No mouth_open / mouth_close layers');
    ...
    if (G) {
      ...
      if (G.eyeL && G.eyeR && !findPart('eye_close').length && anchors.eyeL && anchors.eyeR) {
        ...
        synth.eye = true;
        warnings.push('No eye_close layer - added a generic closed-eye overlay (tune via the Eyes closed-art sliders)');
      }
      // mouth
      if (G.mouth && !findPart('mouth_close').length && byName['mouth_open']) {
        ...
        synth.mouth = true;
        warnings.push('No mouth_close layer - added a generic closed-mouth overlay (tune via the Mouth sliders)');
      }
    }
